...
def ventana_inicio():
    ...
    Label(text="Escoja su opción", bg="Green", width="300", height="2", font=("Calibri", 13)).pack()#ETIQUETA CON TEXTO
    
    Label(text="").pack()
    
    Button(text="Inicar Sesion", height="2", width="30", bg=pestas_color, command=login).pack() #BOTÓN "Acceder"
...
def login():
    ...
    Button(ventana_login, text="Iniciar Sesion", width=10, height=1, command = verifica_login).pack()
...
def verifica_login():
    ...
    if usuario1 in lista_archivos:
        ...
        #SI LA CONTRASEÑA NO SE ENCUENTRA EN EL ARCHIVO....
            ...
            no_clave() #...EJECUTAR "no_clave()"SI EL NOMBRE INTRODUCIDO NO SE ENCUENTRA EN EL DIRECTORIO...
    else:
